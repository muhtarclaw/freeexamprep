...
function QuizRenderer({ quiz }: Props) {
  // ---- Lesen T1 ----
  if (quiz.type === "lesen-t1") {
    const q = quiz as LesenT1Quiz;
    const [selected, setSelected] = useState<number[]>(Array(q.sections.length).fill(-1));
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex gap-4">
          ...
          <button
            onClick={() => { setSelected(Array(q.sections.length).fill(-1)); setRevealed(false); }}
  ...
  if (quiz.type === "lesen-t2") {
    const q = quiz as LesenT2Quiz;
    const [selected, setSelected] = useState<number[]>(Array(q.questions.length).fill(-1));
    const [revealed, setRevealed] = useState(false);
    ...
    return (
      <div className="space-y-6">
        <div className="editorial-card rounded-[1.5rem] p-6">
          ...
          <p className="text-sm leading-relaxed text-[color:var(--foreground)] whitespace-pre-wrap">{q.text}</p>
        </div>
        ...
        <div className="flex gap-4">
          ...
          <button onClick={() => { setSelected(Array(q.questions.length).fill(-1)); setRevealed(false); }}
            className="rounded-full border border-[var(--line)] px-6 py-3 text-sm font-medium">
            Reset
          </button>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="editorial-card rounded-[1.5rem] p-6">
          <p className="text-sm font-medium text-[color:var(--ink-soft)] mb-4">
            Klicken Sie auf eine Anzeige, um sie einer Situation zuzuordnen (in Bearbeitung).
          </p>
        </div>
      </div>
    );
  ...
  if (quiz.type === "sprachbausteine-t1") {
    const q = quiz as SprachbausteineT1Quiz;
    ...
    function renderText() {
      ...
      const parts: string[] = [];
      let lastIndex = 0;
      const blankRegex = /____\d*____|____|(__\d+__)/g;
      let match;
      let blankNum = 0;
      const newText = text.replace(/____\d*____|____/g, () => {
        blankNum++;
        const sel = selected[blankNum];
        const b = q.blanks.find(b => b.id === blankNum);
  ...
  if (quiz.type === "sprachbausteine-t2" || quiz.type === "sprachbausteine-t3") {
    const q = quiz as SprachbausteineT2Quiz;
    const [selected, setSelected] = useState<Record<number, number>>({});
    const [revealed, setRevealed] = useState(false);
      ...
      <div className="space-y-6">
        ...
        {q.exercises.map((exercise, ei) => (
          <div key={ei} className="editorial-card rounded-[1.5rem] p-6">
            ...
            <button
              onClick={() => {
                // Reveal this exercise
              }}
              className="mt-4 text-xs text-[color:var(--brand)] hover:underline"
            >
              Lösung zeigen
            </button>
          </div>
